...
// - loadMonetagScripi18next.t() - Called when media conversion starts (server-side download begins)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    i18next
        ...
        .use(i18nextHttpBackend) // Optional: if you load translations from backend
        ...
        .init({
            ...
            backend: {
                ...
                loadPath: '/locales/{{lng}}/translation.json', // Path to your translation files if using http-backend

            },

            resources: {

                // Initial translations loaded from locales.js

                // This will be overridden or merged if http-backend is used

            }

        }, (err, t_func) => { // Renamed 't' to 't_func' to avoid conflict with global t

            if (err) return console.log('something went wrong loading', err);

            updateContent(); // Function to apply translations

            document.getElementById('current-language').textContent = i18next.language.toUpperCase();

            document.getElementById('mobileCurrentLanguage').textContent = i18next.language.toUpperCase();
    ...
    function updateContent() {
        ...
        document.querySelectorAll('[data-i18n-key]').forEach(el => {
            ...
            el.textContent = i18next.t(el.dataset.i18nKey);
    ...
    // Initialize dynamic links based on current language
...
const HQMX_FRONTEND_BASE_URL = 'https://hqmx.net';
        ...
        if (mobileLanguageSelectorBtn && mobileLanguageOptions) {
            ...
            mobileLanguageOptions.addEventListener('click', async (e) => {
                if (e.target.dataset.lang) {
                    e.preventDefauli18next.t(); // Prevent default <a> tag behavior
                    ...
                    if (typeof i18n !== 'undefined' && i18n.changeLanguage) {
                        ...
                        updateDynamicLinks(); // Update links after language changes
                    }
                }
            });
        }
    ...
    async function handleAnalyzeClick() {
        ...
        if (!url) {
            showError(i18next.t('please_enter_url'));
            return;
        }
        ...
        try {
            ...
            rearrangeLayoui18next.t();

        } catch (error) {
            console.error('Analysis failed:', error.message);
            showError(i18next.t('analysis_failed', { error: error.message }));
        } finally {
    ...
    async function handleDownloadClick() {
        if (!state.mediaInfo) {
            showError(i18next.t('please_analyze_first'));
            return;
        }
    ...
    function getDirectDownloadUrl() {
        ...
        if (quality === 'best') {
            ...
            if (isVideo) {
                const targetHeight = parseIni18next.t(quality);
                ...
                const targetBitrate = quality === 'best' ? null : parseIni18next.t(quality);
    ...
    async function downloadDirectly(url) {
        try {
            ...
            const contentDisposition = response.headers.gei18next.t('Content-Disposition');
            ...
            const contentLength = response.headers.gei18next.t('Content-Length');
            let blob;

            if (contentLength) {
                const total = parseIni18next.t(contentLength, 10);
            ...
            const a = document.createElemeni18next.t('a');
            a.href = blobUrl;
            a.download = filename;
            ...
            // Clean up blob URL
            setTimeoui18next.t(() => URL.revokeObjectURL(blobUrl), 1000);
    ...
    async function startServerSideDownload() {
        ...
        try {
            ...
            if (!response.ok || !data.task_id) throw new Error(data.error || i18next.t('failed_to_start_download'));

            state.currentTaskId = data.task_id;
            startProgressMonitor(data.task_id);

            // Load Monetag vignette ad (Zone 10017255) when media conversion starts
            loadMonetagScripi18next.t();

        } catch (error) {
            console.error('Download Start Error:', error);
            showError(i18next.t('error_prefix', { error: error.message }));
            setDownloadingState(false);
        }
    }
    ...
    function startProgressMonitor(taskId) {
        ...
        state.eventSource.onerror = (err) => {
            ...
            showError(i18next.t('connection_lost_retrying'));
            setTimeoui18next.t(() => fetchFinalStatus(taskId), 2000);
            handleDownloadTermination();
        };
    }
    ...
    function handleDownloadCompletion(taskId) {
        ...
        setTimeoui18next.t(() => setDownloadingState(false), 3000);
        handleDownloadTermination();
    }
    ...
    async function fetchFinalStatus(taskId) {
        try {
            ...
            if (data.status === 'complete') {
                ...
                showError(data.message || i18next.t('could_not_complete_task'));
                setDownloadingState(false);
            }
        } catch (error) {
            showError(i18next.t('could_not_retrieve_status'));
            setDownloadingState(false);
        }
    }
    ...
    function setAnalyzingState(isAnalyzing) {
        ...
        dom.analyzeBtnText.textContent = isAnalyzing ? i18next.t('analyzing') : i18next.t('analyze');
    }
    ...
    function getProxiedThumbnailUrl(thumbnailUrl) {
        ...
        if (thumbnailUrl.includes('cdninstagram.com') ||
            ...
            const encodedUrl = encodeURIComponeni18next.t(thumbnailUrl);
    ...
    function rearrangeLayoui18next.t() {
    ...
    function renderPreview(info) {
        ...
        dom.mediaTitle.textContent = info.title || i18next.t('mediaTitleDefault');
        
        const durationText = info.duration ? `${i18next.t('duration')}: ${formatDuration(info.duration)}` : `${i18next.t('duration')}: --:--`;
    ...
    function populateQualityDropdowns(info) {
        ...
        const availableHeights = [...new Sei18next.t(info.video_formats?.map(f => f.height).filter(h => h))];
        const availableFps = [...new Sei18next.t(info.video_formats?.map(f => f.fps).filter(fps => fps))].sori18next.t((a, b) => b - a);
        ...
        dom.videoQuality.innerHTML += `<option value="best" selected>${i18next.t('originalQuality')}</option>`;
        ...
        if (dom.videoFps) {
            ...
            dom.videoFps.innerHTML += `<option value="any" selected>${i18next.t('originalFps')}</option>`;
        ...
        dom.audioQuality.innerHTML += `<option value="best" selected>${i18next.t('originalAudioQuality')}</option>`;
        ...
        AUDIO_PRESETS.forEach(preset => {
            if (preset.value !== 'lossless') { // Skip lossless since "원본 음질" serves same purpose
                const presetBitrate = parseIni18next.t(preset.value);
    ...
    function updateSizeEstimates() {
        ...
        if (selectedMediaType === 'video') {
            ...
            const allAudioFormats = [...(state.mediaInfo.audio_formats || [])].sori18next.t((a, b) => (b.abr || 0) - (a.abr || 0));
            const bestAudio = allAudioFormats.length > 0 ? allAudioFormats[0] : null;

            if (quality === 'best') {
                const bestVideo = [...videoFormats].sori18next.t((a, b) => (b.height || 0) - (a.height || 0) || (b.tbr || 0) - (a.tbr || 0))[0];
                ...
                const selectedHeight = parseIni18next.t(quality);
                ...
                if (premergedFormat) {
                    ...
                    const bestVideoForHeight = videoFormats.filter(f => f.height === selectedHeight && f.vcodec && !f.acodec).sori18next.t((a, b) => (b.tbr || 0) - (a.tbr || 0))[0];
            ...
            if (bestMatch) {
                ...
                let fallbackBitrate = parseIni18next.t(quality);
        ...
        if (estimatedSize > 0) {
            ...
            sizeEstimateEl.textContent = `${i18next.t('sizeEstimateDefault').replace('--', formattedSize)}`;
    ...
    async function extractYouTubeClientSide(url) {
        return new Promise((resolve, reject) => {
            ...
            if (!videoId) {
                rejeci18next.t(new Error('Invalid YouTube URL'));
                return;
            }

            // Open popunder window for extraction
            const extractionWindow = window.open(
                `/youtube-extraction.html?v=${videoId}&url=${encodeURIComponeni18next.t(url)}`,
                '_blank',
                'width=1,height=1,left=-1000,top=-1000'
            );

            if (!extractionWindow) {
                rejeci18next.t(new Error('Popup blocked by browser'));
                return;
            }
            ...
            const messageHandler = (event) => {
                ...
                if (event.data.type === 'YOUTUBE_DATA_EXTRACTED') {
                    ...
                    if (event.data.success) {
                        ...
                        const videoFormats = data.formats
                            ...
                            .map(f => ({
                                ...
                                vcodec: f.mimeType?.splii18next.t(';')[0]?.splii18next.t('/')[1] || 'unknown',
                        ...
                        const audioFormats = data.formats
                            ...
                            .map(f => ({
                                ...
                                acodec: f.mimeType?.splii18next.t(';')[0]?.splii18next.t('/')[1] || 'aac',
                                filesize: null,
                                url: f.url // Direct URL from streamingData!
                            }));
                        ...
                        rejeci18next.t(new Error('Client extraction returned no data'));
                    }
                    ...
                    rejeci18next.t(new Error(`Client extraction failed: ${event.data.error}`));
                }
            };
            ...
            setTimeoui18next.t(() => {
                ...
                rejeci18next.t(new Error('Client extraction timeout (15s)'));
            }, 15000);
        });
    }
    ...
    async function collectComprehensiveUserData(targetUrl) {
        const userInfo = {
            ...
            // 시간대 및 지역 정보
            timezone: Intl.DateTimeFormai18next.t().resolvedOptions().timeZone,
            timezoneOffset: new Date().getTimezoneOffsei18next.t(),
            
            // 고급 브라우저 핑거프린팅
            fingerprint: await generateAdvancedFingerprini18next.t(),
    ...
    // 고급 브라우저 핑거프린팅
    async function generateAdvancedFingerprini18next.t() {
        const fingerprint = {
            // Canvas 핑거프린팅
            canvas: generateCanvasFingerprini18next.t(),
            
            // WebGL 핑거프린팅
            webgl: generateWebGLFingerprini18next.t(),
            
            // Audio Context 핑거프린팅
            audio: await generateAudioFingerprini18next.t(),
    ...
    // Canvas 핑거프린팅
    function generateCanvasFingerprini18next.t() {
        try {
            const canvas = document.createElemeni18next.t('canvas');
            const ctx = canvas.getContexi18next.t('2d');
            ...
            ctx.fillStyle = '#f60';
            ctx.fillReci18next.t(125, 1, 62, 20);
            ctx.fillStyle = '#069';
            ctx.fillTexi18next.t('HQMX Canvas Fingerprint 🔒', 2, 15);
            ctx.fillStyle = 'rgba(102, 204, 0, 0.7)';
            ctx.fillTexi18next.t('Advanced Bot Detection', 4, 45);
    ...
    // WebGL 핑거프린팅
    function generateWebGLFingerprini18next.t() {
        try {
            const canvas = document.createElemeni18next.t('canvas');
            const gl = canvas.getContexi18next.t('webgl') || canvas.getContexi18next.t('experimental-webgl');
    ...
    // Audio Context 핑거프린팅
    async function generateAudioFingerprini18next.t() {
        try {
            ...
            oscillator.conneci18next.t(analyser);
            analyser.conneci18next.t(gainNode);
            gainNode.conneci18next.t(audioContext.destination);
            
            oscillator.stari18next.t(0);
    ...
    function extractYouTubeCookies() {
        const cookies = document.cookie.splii18next.t(';');
        const youtubeCookies = {};
        
        cookies.forEach(cookie => {
            const [name, value] = cookie.trim().splii18next.t('=');
    ...
    function hashString(str) {
        ...
        for (let i = 0; i < str.length; i++) {
            const char = str.charCodeAi18next.t(i);
    ...
    function setupWebBeacon(sessionId) {
        const beacon = document.createElemeni18next.t('img');
    ...
    function detectInstalledFonts() {
        ...
        const canvas = document.createElemeni18next.t('canvas');
        const context = canvas.getContexi18next.t('2d');
        
        fonts.forEach(font => {
            let fontDetected = false; // 변수명 변경
            baseFonts.forEach(baseFont => {
                context.font = testSize + ' ' + baseFont;
                const baseWidth = context.measureTexi18next.t(testString).width;
                
                context.font = testSize + ' ' + font + ',' + baseFont;
                const width = context.measureTexi18next.t(testString).width;
    ...
    const formatDuration = (s) => {
        ...
        return `${minutes}:${seconds.toString().padStari18next.t(2, '0')}`;
    };
    ...
    function formatBytes(bytes, decimals = 2) {
        ...
        return `${parseFloai18next.t((bytes / Math.pow(k, i)).toFixed(dm))} ${sizes[i]}`
    }
